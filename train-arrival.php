<!DOCTYPE html>
<html>
<head>
	<link href="style.css" type="text/css" rel="stylesheet" />
	<!--<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css" />-->
	<title>WebMS | Train Arrival</title>
</head>
<body>
	<?php

			include_once 'config.php';

			$_SESSION['messsage'] = '';

				$monthNum = date("m");
				$monthName = date('F',mktime(0,0,0,$monthNum, 10));

			if ($_SERVER['REQUEST_METHOD'] == 'POST') {

				$time_in = $mysqli->real_escape_string($_POST['time_in']);
				$time_out = $mysqli->real_escape_string($_POST['time_out']);
				$station_name = 'Tutuban';
				$train_number = $mysqli->real_escape_string($_POST['train_number']);
				$date_departed = $monthName . " " . date('d') . ", " . date('Y');

				$sql = "INSERT INTO tb_tarrival (station_name, train_number, time_in, time_out, date_departed) "
				. "VALUES ('$station_name, $train_number, '$time_in','$time_out','$date_departed')";

				if ($mysqli->query($sql) === true) {

					$_SESSION['message'] = '<p class="c-panel-ta-message-good">Record has been saved!</p>';
					header("refresh: 2; url=train_arrival.php");
				}
			}

			$sql2 = "SELECT * FROM tb_tarrival";

			$result2 = $mysqli->query($sql2);


	?>
	<!--Eskriptong Para sa Pangyayari at Animasyon-->
	<script type="text/javascript" src="main.js">
	</script>

	<!--Unang Kabanata-->
	<div id="sidebar" class="s-panel">
		<div class="s-panel-p">
			<img <?php echo 'src="'.$_SESSION['user_img'].'"' ?> />
			<h4><?= $_SESSION['fname']. " " .$_SESSION['lname'] ?></h4>
			<?php

			if($_SESSION['gender'] == 'Male') {
				echo '<p class="gender-boy">@'.$_SESSION['username']. '</p>';
			}
			else {
				echo '<p class="gender-girl>@'.$_SESSION['username']. '</p>';
			}
			?>
		</div>
		<div class="s-panel-o">
			<ul type="none">
				<li><a href="dashboard.php"><i class="fa fa-home"></i>Dashboard</a></li>
			</ul>
			<p>POINT OF SALE OPTIONS</p>
			<ul type="none">
				<li><a href="transaction.php"><i class="fa fa-credit-card"></i>Transactions</a></li>
				<li><a href="inventory.php"><i class="fa fa-archive"></i>Ticket Inventory</a></li>
				<li><a href="sales-monitoring.php"><i class="fa fa-bar-chart"></i>Sales Monitoring</a></li>
			</ul>
			<p>INFORMATION GATHERING OPTIONS</p>
			<ul type="none">
				<li><a href="information.php"><i class="fa fa-book"></i>Information</a></li>
				<li><a href="news.php" class="s-panel-o-i"><i class="fa fa-newspaper-o"></i>Create Article</a></li>
				<li><a href="announcement.php" class="s-panel-o-i"><i class="fa fa-bullhorn"></i>Create Event</a></li>
				<li><a href="train-arrival.php" class="s-panel-o-active"><i class="fa fa-train"></i>Train Arrival</a></li>
				<li><a href="feedback.php"><i class="fa fa-comments"></i>User Feedback</a></li>
			</ul>
			<p>PERSONAL INFORMATION OPTIONS</p>
			<ul type="none">
				<li><a href="background.php"><i class="fa fa-globe"></i>Company Background</a></li>
				<li><a href="developers.php"><i class="fa fa-group"></i>The Developers</a></li>
			</ul>
		</div>
	</div>
	<div id="content" class="c-panel-ta c-panel">
		<div class="c-panel-h">
			<div class="c-panel-h-mi">
				<img src="img/menu-icon-white.png" id="menu-icon" onclick="rotateMenuIcon()" />
			</div>
			<h1>Web<span>MS</span></h1>
			<ul type="none">
				<li><a href="logout.php" class="c-panel-h-a-notif"><i class="fa fa-user"></i> Logout</a></li>
				<li><a href="#" class="c-panel-h-a-notif"><i class="fa fa-bell"></i> Notification</a></li>
				<li><a href="#" class="c-panel-h-a-notif"><i class="fa fa-comment"></i> Conversation</a></li>
			</ul>
		</div>
		<div class="c-panel-ta-add">
			<div class="c-panel-ta-add-h">
				<h1>Create Schedule</h1>
			</div>
			<form action="train-arrival.php" method="post">
				<label>Time In:</label>
				<input type="time" name="time_in" required />
				<label>Time Out:</label>
				<input type="time" name="time_out" /> 
				<label>Station Name:</label>
				<select name="station_name" class="c-panel-ta-add-station" disabled >
					<option value="Tutuban">Tutuban</option>
				</select>
				<label>Train Number:</label>
				<select name="train_number" required />
					<option>1</option>
					<option>2</option>
					<option>3</option>
				</select>
				<label>Date Departed:</label>
				<select name="date_depart_m" disabled="disabled">
					<option><?php echo date("M") ?></option>
				</select>
				<select name="date_depart_d" disabled="disabled">
					<option><?php echo date("d") ?></option>
				</select>
				<select name="date_depart_y" disabled="disabled">
					<option><?php echo date("Y") ?></option>
				</select>
				<input type="submit" value="Create Schedule" />
			</form>
		</div>
		<div class="c-panel-ta-time">
			<div class="c-panel-ta-time-h">
				<h1>Train Arrival</h1>
			</div>
			<table cellspacing="0">
				<tr>
					<th>ID</th>
					<th>Station</th>
					<th>Time In</th>
					<th>Time Out</th>
					<th>Date Departed</th>
				</tr>
				<!--<tr>
					<td>1</td>
					<td>Tutuban</td>
					<td>05:30</td>
					<td>20:15</td>
					<td>December 30, 2018</td>
					<td><a href="#"><i class="fa fa-pencil"></i>Update</a></td>
					<td><a href="#"><i class="fa fa-eye"></i>View</a></td>
				</tr>-->
				<?php
					while ($row = mysqli_fetch_array($result2)) {
						echo '<tr>';
						echo '<td>'.$row['arrival_id'].'</td>';
						echo '<td>'. $row['station_name'] .'</td>';
						echo '<td>'.$row['time_in'].'</td>';
						echo '<td>'.$row['time_out'].'</td>';
						echo '<td>'.$row['date_departed'].'</td>';
						echo '</tr>';
					}
				?>
			</table>
		</div>
		<div class="c-panel-ta-page">
			<ul type="none">
				<li><a href="#">&laquo;</a></li>
				<li><a href="#" class="c-panel-ta-page-active">1</a></li>
				<li><a href="#">2</a></li>
				<li><a href="#">3</a></li>
				<li><a href="#">4</a></li>
				<li><a href="#">5</a></li>
				<li><a href="#">&raquo;</a></li>
			</ul>
		</div>
	</div>
</body>
</html>